if (typeof loadMoreFriend === 'function') {
	$(document).on('click', '.btn-load-more', loadMoreFriend);
}
...
jQuery(document).ready(function ($) {
	...
	$('.top-area > .setting-area > li > a').on("click", function (event) {
		event.preventDefault(); // Ngăn hành vi mặc định của thẻ <a>
		event.stopPropagation(); // Ngăn sự kiện truyền lên các phần tử cha

		// Xử lý cho dropdown liên quan đến mục bạn click
		var $parent = $(this).parent('li');
		$(this).toggleClass('active').parent().siblings().children('a').removeClass('active');
		$parent.siblings().children('div').removeClass('active');
		$(this).siblings('div').toggleClass('active'); // Hiển thị hoặc ẩn dropdown
	});

	// Đảm bảo click bên ngoài dropdown sẽ đóng nó
	$("body").on("click", function (event) {
		if (!$(event.target).closest('.top-area > .setting-area > li').length) {
			$(".top-area > .setting-area > li > a").removeClass('active');
			$(".top-area > .setting-area > li > div").removeClass('active');
		}
	});

	// Nếu click vào nội dung trong dropdown (ví dụ, link "View All"), không bị ảnh hưởng bởi sự kiện khác
	$('.top-area > .setting-area > li .dropdowns a').on("click", function (event) {
