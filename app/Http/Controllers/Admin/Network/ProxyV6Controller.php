...
use App\Repositories\Network\ProxyV6\ProxyV6RepositoryInterface;
...
class ProxyV6Controller extends Controller
{
    ...
    {
        return view('admin.pages.Netwrork.proxy_system');
    }
    ...
    public function create(Request $request)
    {
		...
		{
			$RandomPassword = true;
			$networkInterface = 'enp6s0.ip1.pppo';
    ...
    public function ReloadIpv6(string $id)
    {
		...
		if ($result) {
			...
			if($resultCreate->status)
			{
				...
				$testFile = $GenProxy->makeFileConfigProxy($DefaultPath."/ProxyV6_{$configName}.cfg", $interface, '0.0.0.0', $result->port, $result->username, $result->password, $result->access_token);
    ...
    public function CheckProxy(string $id)
    {
		...
		if ($apiResponse === false) {
            return response()->json(['status' => false, 'msg' => 'Không thể kết nối tới API thông qua proxy'], 500);
		}
