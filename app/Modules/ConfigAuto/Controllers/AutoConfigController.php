...
namespace App\Modules\ConfigAuto\Controllers;
...
class AutoConfigController extends Controller
{
    ...
    public function interactLimit()
    {

        $limit = $this->configAutoRepository->findByInteractLimit('interact_limit');

        

        $defaultLimit = config('defaultconfigs.defaultConfigInteractLimit');

        $configLimit = (isset($limit['interact_limit'])) ? $limit['interact_limit'] : $defaultLimit;

        return view('ConfigAuto::action_limit_page', compact('configLimit'));
    }

   
    public function saveInteractLimit(Request $request)
    {
        // Lấy dữ liệu từ form
        $data = $request->all();
        // Lưu cấu hình interact_limit (nếu có record thì cập nhật, nếu chưa có thì thêm mới dựa vào default)
        $this->configAutoRepository->saveInteractLimit($data);

        // Lấy lại cấu hình vừa lưu
        $interactLimitData = $this->configAutoRepository->findByInteractLimit('interact_limit');
        if (isset($interactLimitData['_id'])) {
            unset($interactLimitData['_id']);
        }

        // Cập nhật cấu hình interact_limit cho tất cả các tài khoản
        $updateResult = $this->accountRepository->updateInteractLimit($interactLimitData);

        return redirect()->route('facebook.action.limit.page')
        ->with('success', 'Cấu hình đã được lưu và áp dụng cho tất cả tài khoản!');
    }
}
