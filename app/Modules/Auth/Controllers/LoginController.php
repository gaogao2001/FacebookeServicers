...
class LoginController extends Controller
{
    ...
    // Hàm kiểm tra IP có phải là Local hay không
    private function isLocalIp($ip)
    {
        return filter_var(
            ...
            (
                ...
                substr($ip, 0, 8) === '192.168.' ||    // Private IPv4 range
                substr($ip, 0, 7) === '10.' ||         // Private IPv4 range
                (substr($ip, 0, 3) === '172' &&        // Private IPv4 range (172.16.0.0 - 172.31.255.255)
    ...
    public function register(Request $request)
    {
        ...
        if ($this->isLocalIp($ClientIP)) {
            ...
            if (!$createdRoleId) {
                ...
                $createdRole = $this->roleRepository->create($roleData);
            ...
            $token = hash_hmac('sha256', $data['email'] . $now, $secretKey);
            ...
            return redirect('/login')->withErrors(['error' => 'Khác IP chim cook :))']);
        }
    }
    ...
    {
        $clientIp = $request->ip();
        // Ví dụ: hàm kiểm tra local IP như đã có
        $isLocal = filter_var(
            $clientIp,
            FILTER_VALIDATE_IP,
            FILTER_FLAG_IPV4 | FILTER_FLAG_IPV6
        ...
        return response()->json(['message' => 'IP hợp lệ.'], 200);
    }
}
